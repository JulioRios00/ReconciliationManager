...
from models.schema_dlm import Documento, TipoDocumento, TagDocumento, PalavraChave, PalavraChaveDocumento, DocumentoAnexo
from repositories.dlm_repository import *
from repositories.work_flow_repository import TagWorkFlowRepository, TagAvisoRepository
...
class DocumentoApiService():
    ...
    def list_document_type_by_cnpj(self, cnpj, page, per_page):
        response = self.tipo_documento_repository.get_all_by_cnpj(cnpj, page, per_page)
        return jsonify(response)

    def create_document_types(self,company_id,extensao,nome,sigla):
        obj_dup = self.tipo_documento_repository.get_first([TipoDocumento.Nome == nome, TipoDocumento.IdEmpresa == company_id])
        if obj_dup:
            raise CustomException((X_ALREADY_EXISTS).format('Document type'))
        obj = TipoDocumento(company_id,extensao,nome,sigla)
    ...
    def create_document(self, key, document_type_id, descricao):
        ...
        obj = Documento(document_name, key, f'{tipo_documento.Sigla}-{count+1}', 1, 0, 0, True, status, document_type_id, descricao)
        self.tipo_documento_repository.add(obj)
        return CREATED, 201
    ...
    def create_tag_document(self, document_id, tag_id):
        ...
        if tag_doc_obj:
            ...
            tag_doc = TagDocumento(tag_id, document_id)
    ...
    def validate_tag_document(self,document_id,tag_id,validation, document_type_id):
        ...
        for item in tag_wf_obj:
            ...
            if tag_dest_id:
                next_tag = TagDocumento(item.IdTagDest,document_id)
    ...
    def validate_document(self, document_id, status):
        ...
        if status == True:
            ...
            for branch in work_flow:
                obj_tag_document = TagDocumento(branch.IdTagDest, documento.Id) if branch.IdTagOrig == str(branch.IdTagDest) else obj_tag_document
    ...
    def create_doc_anexo(self, id_documento, id_anexo):
            ...
            return jsonify(insert_obj.serialize())
    ...
    def send_email(self, id_documento, list_emails, email_subject, email_body):
        if id_documento: #tag rejeitada.
            ...
            self.enviar_email(list_emails, email_subject, email_body)
        else:
