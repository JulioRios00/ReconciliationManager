...
import uuid
from .base import Base
from sqlalchemy import Column, TIMESTAMP, Boolean, text, ForeignKey, Text, Integer, String, Date
from sqlalchemy.dialects.postgresql import UUID
from sqlalchemy.orm import relationship
import uuid
...
class CategoriaArquivo(Base):
    ...
    IdCategoria = Column(UUID(as_uuid=True), ForeignKey('CategoriaEmpresa.Id'), index=True)
...
class ArquivoUploadEmpresa(Base):
    ...
    def __init__(self, IdEmpresa, TipoArquivo, Qtd, Extensao, DataInicio=None, DataFim=None):
